...
(function( $ ){
	...
	var methods = {
		...
		init : function( options  , id)
		{
			...
			var menuGroups = {'dbenter' : ['dbitems'],
			...
			var $this = $(this).hide();	       	
	       	...
	       	var menuBar = $( "<div/>",{ id : "menuBarDiv",
								  		menuBarDivclass : "row-fluid"
							}).prependTo('#editBarText');
			...
			var statusBar = $("<div/>", {	id : "statusbar",
	       	...
	       	//menuBar.append("<strong>Format Text</strong>");
	       		...
	       		if(item in menuGroups){
	       			...
	       			for(var index=0;index<menuGroups[item].length;index++){
	       				...
	       				if(settings[value]){
							...
							group.append($("<div/>",{class:"btn-group"}));
       					}
		...
		createMenuItem: function(itemSettings, options, returnElement){
			...
			if(itemSettings["select"]){
				...
				menuWrapElement.append($('<a/>',{
										class:"btn active dropdown-toggle",
				...
				var menuWrapElement = $("<a/>",{href:'javascript:void(0)', class:'btn active'});
				var menuElement = $("<i/>");
				if(itemSettings["icon"])
					menuElement.addClass(itemSettings["icon"]);
				else
